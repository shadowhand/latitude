...
class ConditionsTest extends TestCase
{
    ...
    {
        $conditions = Conditions::make()->with('role_id IN ?', InValue::make([1, 2, 3]))->orWith('user_id IN ?', InValue::make([100]));
        $this->assertSql($conditions, 'role_id IN (?, ?, ?) OR user_id IN (?)');
        $this->assertParams($conditions, [1, 2, 3, 100]);
        $conditions = Conditions::make()->with('role_id IN ?', InValue::make([4, 5, 6]));

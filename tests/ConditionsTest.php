...
class ConditionsTest extends TestCase
{
    ...
    {
        $conditions = Conditions::make()->with('id = ?', 1)->andWith('last_login > ?', 'today')->orWith('last_login IS NULL');
    ...
    {
        $conditions = Conditions::make()->with('role_id IN ?', ValueList::make([1, 2, 3]))->orWith('user_id IN ?', ValueList::make([100]));
        $this->assertSql($conditions, 'role_id IN (?, ?, ?) OR user_id IN (?)');
        $this->assertParams($conditions, [1, 2, 3, 100]);
        $conditions = Conditions::make()->with('role_id IN ?', ValueList::make([4, 5, 6]));
        $this->assertSql($conditions, 'role_id IN (?, ?, ?)');
        $this->assertParams($conditions, [4, 5, 6]);
    }
    public function testCombinedStatement()
    {
        $conditions = Conditions::make()->with('id IN ? OR id = ?', ValueList::make([5, 10]), 1);
        $this->assertSql($conditions, 'id IN (?, ?) OR id = ?');
        $this->assertParams($conditions, [5, 10, 1]);
    }
    public function testGroupingWithAnd()
    {
        $conditions = Conditions::make()->with('id = ?', 1);
        $group = $conditions->group()->with('last_login > ?', 'today')->orWith('last_login IS NULL');
    ...
    {
        $conditions = Conditions::make()->group()->with('failed_logins > ?', 5)->andWith('last_login IS NULL')->end()->orGroup()->with('role = ?', 'banned')->end();
    ...
    public function testSubConditionIdentifier()
    {
        ...
        $conditions = Conditions::make()->with('u.id = ?')->orGroup()->with('u.username = ?')->end();
        $this->assertSame('"u"."id" = ? OR ("u"."username" = ?)', $conditions->sql($identifier));
    }
    private function assertSql(Conditions $conditions, $expected)
    {
